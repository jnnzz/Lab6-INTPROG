...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
  ...
  if (origin) {
    ...
    message = `<p>If you don't know your password please reset it with the /account/forgot-password API route.</p>`;
  }
...
async function sendPasswordResetEmail(account: any, origin: any) {
  ...
  if (origin) {
    ...
    message = `<p>Please use the below token to reset your password with the /account/reset-password API route:</p><p><code>${account.resetToken}</code></p>`;
  }
